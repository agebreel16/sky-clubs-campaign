...
{{-- ── Hero Banner ── --}}
<div style="background: linear-gradient(135deg, #b45309 0%, #f59e0b 100%); border-radius: 16px; padding: 24px; box-shadow: 0 8px 24px rgba(180,83,9,0.25); margin-bottom: 0;">
    <div style="display:flex; flex-wrap:wrap; align-items:center; justify-content:space-between; gap:16px;">

        <div style="flex:1; min-width:0;">
            <p style="margin:0 0 8px; font-size:11px; font-weight:700; letter-spacing:2px; text-transform:uppercase; color:rgba(254,243,199,0.9);">
                API الموزعين
            </p>
            ...
            @if ($sync)
                <div style="margin-top:10px; display:flex; flex-wrap:wrap; gap:4px 20px; font-size:13px; color:rgba(254,243,199,0.85);">
            ...
            @else
                <p style="margin:10px 0 0; font-size:13px; color:rgba(254,243,199,0.75);">لم تُجرَ أي مزامنة بعد</p>
            @endif
        </div>
...
<div style="background:#fff; border:1px solid #e5e7eb; border-radius:16px; overflow:hidden; box-shadow:0 1px 4px rgba(0,0,0,0.06);">
    ...
    <div style="padding:16px 24px; border-bottom:1px solid #f3f4f6; background:#f9fafb;">
        ...
        <p style="margin:4px 0 0; font-size:12px; color:#6b7280;">
            يجب أن يُرجع API مصفوفة موزعين — المعرّف الفريد هو
            <code style="background:#e5e7eb;padding:1px 5px;border-radius:4px;font-size:11px;">phone</code>
        </p>
    </div>
    ...
    <div style="padding:24px; display:flex; flex-direction:column; gap:20px; max-width:560px;">
        ...
        <div>
            ...
            <x-filament::input.wrapper>
                <x-filament::input type="url" wire:model="api_url" placeholder="https://example.com/api/distributors" dir="ltr" />
            </x-filament::input.wrapper>
        </div>
        ...
        <div x-data="{ show: false }">
            ...
            <x-filament::input.wrapper>
                ...
                <x-slot name="suffix">
                    <button type="button" @click="show=!show" style="font-size:12px;font-weight:500;color:#6b7280;padding:0 12px;cursor:pointer;white-space:nowrap;border:none;background:none;transition:color .15s;" onmouseover="this.style.color='#b45309'" onmouseout="this.style.color='#6b7280'">
        ...
        <div style="padding-top:12px; border-top:1px solid #f3f4f6;">
            <x-filament::button wire:click="save" wire:loading.attr="disabled" color="warning">
